...
$format = preg_replace_callback($pattern, function ($matches) use ($placeholders) {
    ...
    public function pdfCollections(Request $request)
    {
       ...
       $query = '{
        ...
        $settingsData = Settings::select('id', 'shop_id', 'collectionId', 'enabled', 'isPdf', 'collectionName', 'pdfUrl')
    ...
    public function pdfShow(Request $request)
    {
       ...
       $post = $request->input();
        ...
        if ($settings) {
            ...
            return response()->json(['status' => 'true', 'pdfName' => $settings['pdfUrl']]);
        } else {
    ...
    public function reauth(){
        $allUser =  User::where('needs_update',0)->get();
