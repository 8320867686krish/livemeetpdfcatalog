...
class ApiController extends Controller
{
    ...
    public function getProductsByCollections(Request $request)
    {
        ...
        while (count($products) < $plan_limit) {
            ...
            foreach ($normalizedCollectionIds as $collectionId) {
                ...
                $promises[$collectionId] = $client->postAsync($shopifyUrl, [
                    'headers' => $headers,
                    'json' => ['query' => $query],
                ]);
            }
            ...
            foreach ($results as $collectionId => $result) {
                if ($result['state'] === 'fulfilled') {
                    $responseData = json_decode($result['value']->getBody(), true);
    ...
    private function buildProductQuery($collectionId, $cursor = null, $variantQuery = '')
    {
        $afterPart = $cursor ? ', after: "' . $cursor . '"' : '';
        return '{
            collection(id: "gid://shopify/Collection/' . $collectionId . '") {
    ...
    public function transformProduct($product, $isProductWithVariant = false)
    {
        ...
        if (!empty($product['variants']['edges'])) {
            if ($isProductWithVariant) {
                ...
                if ($firstVariant) {
                    $normalizedVariantId = preg_replace('/.*\/(\d+)$/', '$1', $firstVariant['id']);
                    $productData['variants'][] = [
                        'id'                  => $firstVariant['id'],
                        'normalizedId'        => $normalizedVariantId,
                        'title'               => $firstVariant['title'] ?? null,
                        'price'               => $firstVariant['price'] ?? null,
                        'compareAtPrice'       => $firstVariant['price'] ?? null,
                        'product'             =>  $product['id'] ?? null,
                        'normalizedProductId' => $normalizedProductId
                    ];
                }
