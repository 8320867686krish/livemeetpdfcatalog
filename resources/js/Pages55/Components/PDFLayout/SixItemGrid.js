...
const DrawGridLayout = (data = {}) => {
    ...
    for (let i = 0; i < productItem.length; i += 2) {
        rows.push(
            <div key={`product_${i / 2}`} className="row" style={{ height: "33.33%" }}>
                <div className="column-wrapper">
                    ...
                    {productItem.slice(i, i + 2).map(pItem => {
                        ...
                        return (<Fragment key={`column_key_${pItem?.id !== undefined ? pItem.id : 2 ** ++j}`}>
                            {Object.keys(pItem).length > 0
                                ?
                                <div className="column column-2" style={{ margin: "10px", backgroundColor: productBackgroundColor, color: productAttributeLabelColor }}>
