...
const DrawGridLayout = (data = {}) => {
    ...
    rows.push(
        <div className="row" style={{ height: "44%", margin: "0 5px", backgroundColor: productBackgroundColor, color: productAttributeLabelColor }}>
            <div style={{ display: "flex", flex: "1 1 auto", flexDirection: "column" }}>
                <ProductImageLayout {...{ productImage: (productItem[0]['image'] === '' || productItem[0]['image'] === null) ? `${IMAGE_PREFIX}images/no_image.png` : productItem[0]['image'], productAttributes, barCode: productItem[0]['barCode'] }} />
    ...
    for (let i = 1; i < productItem.length; i += 2) {
        rows.push(
            <div key={`product_${i / 2}`} className="row" style={{ height: "27%" }}>
                    ...
                    {productItem.slice(i, i + 2).map(pItem => {
                        ...
                        return (<Fragment key={`column_key_${id !== undefined ? id : 2 ** ++j}`}>
                            {Object.keys(pItem).length > 0
                                ?
                                <div className="column column-2 " style={{ width: "50%", margin: "10px 5px", backgroundColor: productBackgroundColor, color: productAttributeLabelColor }}>
                                    <div style={{ display: "flex", flex: "1 1 auto", flexDirection: "column" }}>
                                ...
                                :
                                <div className="column column-2 " style={{ width: "50%", margin: "10px" }}>
                                    <div style={{ display: "flex", flex: "1 1 auto", flexDirection: "column" }}>
                                        <ProductImageLayout />
                                    </div>
                                </div>
