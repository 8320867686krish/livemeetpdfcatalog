...
const DrawGridLayout = (data = {}) => {
    ...
    for (let i = 0; i < productItem.length; i += 3) {
        rows.push(
            <>
                <div key={`product_${i / 2}`} className="row" style={{ height: "14.85cm" }}>
                        ...
                        {productItem.slice(i, i + 2).map(pItem => {
                            ...
                            return (<Fragment key={`column_key_${id !== undefined ? id : 2 ** ++j}`}>
                                {Object.keys(pItem).length > 0
                                    ?
                                    <div className="column column-2 " style={{ margin: "0 5px", backgroundColor: productBackgroundColor, color: productAttributeLabelColor }}>
                                    ...
                                    :
                                    <div className="column column-2 " style={{ margin: "0 5px" }}>
                                        <div style={{ display: "flex", flex: "1 1 auto", flexDirection: "column" }}>
                                            <ProductImageLayout />
                                        </div>
                                    </div>
                ...
                {
                    Object.keys(productItem[2]).length > 0
                        ?
                        <div key={`product_${i ** ++j}`} className="row" style={{ height: "14.85cm", margin: "10px 5px", backgroundColor: productBackgroundColor, color: productAttributeLabelColor }}>
                            <div style={{ display: "flex", flex: "1 1 auto", flexDirection: "column" }}>
                                <ProductImageLayout {...{ productImage: (productItem[2]['image'] === '' || productItem[2]['image'] === null) ? `${IMAGE_PREFIX}images/no_image.png` : productItem[2]['image'], productAttributes, barCode: productItem[2]['barCode'] }} />
                                <ProductContentLayout {...{ valueColor: productAttributeValueColor, productPageLayoutId, fontColor, backgroundColor, productButtonEnabled, priceAdjustment, productChangeInPercentage, productTaxPercentage, productAttributeAlignment, productAttributes, descriptionCharLimit: productDescriptionCharLimit, productData: productItem[2] }} />
                            </div>
                        </div>
                        :
                        <div className="row" style={{ height: "14.85cm", marginTop: "10px" }}>
                            <div style={{ display: "flex", flex: "1 1 auto", flexDirection: "column" }}>
                                <ProductImageLayout />
                            </div>
                        </div>
...
const ThreeItemGridReverse = (props) => {
    let {
        innerPagePadding = '',
        pdfWidth = 21,
        pdfHeight = 29.7,
