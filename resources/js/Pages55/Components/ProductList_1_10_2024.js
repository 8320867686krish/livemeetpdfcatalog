...
const ProductList = ({ shopid = '', productLimit = "5", selectedResources = [], parentStateUpdateByChild, collectionId = 0, fetchedProduct = '' }) => {
    ...
    const getProduct = async (curObj = {}) => {
        const { currentState = '', query = '', limit = showLimit, collectionId = 0 } = curObj;
        setLoader(true);
        const request = { startCursor: '', endCursor: '', query, limit, collectionId }
            ...
            <div className="table_area">
                ...
                {(productList.length > 0) &&
                    <>
                        ...
                        {hasNextPage && <div className="show_more"><Button variant="plain" onClick={() => getProduct({ currentState: "next", query: queryValue , collectionId : collectionId })}>Show more products</Button></div>}
                    </>
                }
            </div>
        </LegacyCard>
