...
const Home = (props = {}) => {
    const { shopid = '', activePlan: { catelog_limit = "2", planExpired = false } = {} } = props;
    ...
    const getCatalogCollections = async () => {
        ...
        const { responseCode = '', errorCode = 0, isPayment = false, message = '', theam_setting_url = '', data: { pdfCollections = [] } = {} } = responseData;
    ...
    const deleteSelectedRecord = async () => {
        ...
        const responseData = await fetchMethod('POST', 'collections/remove', shopid, request);
    ...
    const enabledDisabledCatalog = async (status) => {
        ...
        const responseData = await fetchMethod('POST', 'collections/status', shopid, request);
        const { responseCode = '', message = '' } = responseData;
        if (responseCode === 0) {
    ...
    const rowMarkup = catalogList.map((row, index) => {
        ...
        pdfUrl = pdfUrl !== '' ? `${IMAGE_PREFIX}uploads/pdfFile/shop_${shop_id}/collections_${collectionName}/${pdfUrl}` : '';
        ...
        return (
            <IndexTable.Row id={id} key={id} selected={selectedRow} disabled={index >= Number(catelog_limit) || catelog_limit === 'false'}>
                ...
                <IndexTable.Cell>
                    <div className="btn_area">
                        {!(index >= Number(catelog_limit) || catelog_limit === 'false') && <Button variant="primary" onClick={() => navigate(`${URL_PREFIX}dashboard?id=${id}`, { state: { planExpired } })}>Edit</Button>}
                        {/* <Button variant="primary" onClick={() => navigate(`${URL_PREFIX}pdf-preview-flip`, { state: { pdfUrl } })}>Flip Preview</Button> */}
                        <Button tone="success" variant="primary" onClick={(e) => previewPDF(e, pdfUrl)}>PDF Preview</Button>
                        <Button onClick={(e) => downloadPDF(e, pdfUrl)}>Download PDF</Button>
                    </div>
                </IndexTable.Cell>
            </IndexTable.Row>
        )
    ...
    return (
        <Page
            ...
            primaryAction={<Button variant="primary" size="large" disabled={catalogList.length >= Number(catelog_limit) || catelog_limit === 'false' /* || planExpired */} onClick={(catalogList.length >= Number(catelog_limit) || catelog_limit === 'false' /* || planExpired */) ? () => { } : () => navigate(`${URL_PREFIX}dashboard`, { state: { planExpired, totalCatelog: catalogList.length } })}>Create a New Catalog</Button>}
            secondaryActions={[
                {
                    ...
                    <IndexTable
                        ...
                        emptyState={<TableNoRecord
                            emptyProps={{
                                ...
                                message: "Create a new product catalog to get started!",
